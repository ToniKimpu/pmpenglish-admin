...
import { useCreatePatternExerciseVocabulary } from "@/features/SpokenPattern/hooks/usePatternVocabulary";

const CreatePatternExerciseVocabularyDialog = ({ patternExerciseId }) => {
  const [isOpen, setIsOpen] = useState(false);

  const {
    mutate: createPatterExerciseVocabulary,
    isLoading: creating,
    isError,
  } = useCreatePatternExerciseVocabulary(patternExerciseId);
  ...
  const onSubmit = (data) => {
    createPatterExerciseVocabulary(
      {
        exercise_id: patternExerciseId,
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <button
          type="button"
          className="text-appColor bg-white hover:bg-gray-100 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800"
        >
...
export default CreatePatternExerciseVocabularyDialog;
